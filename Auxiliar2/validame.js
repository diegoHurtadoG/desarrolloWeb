function shrek() {
    ...
    let errores = "";
    let nombre = document.getElementsByName("nombre")[0].value; // [0, 1]
    console.info(nombre);
    let regex = /^[a-zA-Z ]*$/;
    // alert(nombre);
    if (nombre.length < 10 || nombre.length > 100 || !regex.test(nombre)) {
        errores += "<p>El nombre esta malo</p>Modifica tu formilario <a href='hackbox.html'>enviar</button>";
...
function procesacolor(){

    let selector = document.getElementById("hola");
...
function insertacosa() {

    if (agregados > 5) {
        alert("oye para");
        return;
    }
    let c = document.getElementById("dinamico");
    c.innerHTML += "<button onclick='botonhack()'>hackeado por pablo la nasa ira por ti</button>";
    agregados += 1;
